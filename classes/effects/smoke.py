...
class Smoke(pygame.sprite.Sprite):
    def __init__(self, *groups, fps=60):
        ...
        self.counter = 0
        self.fps = fps
        SoundManager.load_and_play('blade.wav')

    def update(self, *args, **kwargs):
        # Incrementa o contador para controlar o tempo entre os quadros
        self.counter += 1
        if self.counter >= self.fps*0.01:  # Ajusta o tempo baseado nos FPS
            self.counter = 0
            self.actual_frame += 1

            # Atualiza a imagem com o próximo quadro
            self.image = self.smoke_frames[0][self.actual_frame % len(self.smoke_frames[0])]
